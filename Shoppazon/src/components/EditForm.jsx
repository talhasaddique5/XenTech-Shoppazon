...
const EditForm = ({ setShowEditForm, contract, signer, itemId }) => {
    ...
    if (!title || !description || !category || !image || !price || !stock) {
      ...
      try {
        const shouldUpdate = window.confirm(
          "Are you wure you want to update this product?"
        );
        if (shouldUpdate) {
          const updateProduct = await contract.updateProduct(
            itemId,
            title,
            category,
            description,
          ...
          const txHash = await updateProduct.hash;
          alert(
            `Your Product is Updated Successfully. Here is your Transaction Hash: ${txHash}`
          );
          window.location.reload();
        }
      } catch (error) {
        console.error("Error Updating product:", error);
        alert("Got Some Error While Updating Your Product");
      }
    }
  };
